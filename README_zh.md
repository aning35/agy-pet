...
### 边缘吸附进度条状态
<p align="center">
  <img src="docs/screenshots/agy-pet-docked.png" width="800" />
</p>
...
- 📌 **边缘吸附停靠**：将宠物拖拽到屏幕任何边缘（左右上方），它会自动吸附并折叠成一个细长的状态进度条，绝不遮挡您的工作视线。现在已完美支持 Windows 与 macOS 双端。
- 📊 **本地数据看板**：右键点击并选择“Show Dashboard”，即可查看今日 AI 思考总时长、已修复错误数、在线时间以及消耗的 Token 预估量（基于字符统计）。所有数据纯本地计算，零大模型推理开销。
- 💧 **健康喝水提醒**：内置固定时间的健康喝水语音提醒（9:00, 10:00, 11:00...），在漫长的编码会话中守护你的健康。
- 💾 **智能位置记忆**：宠物会自动记忆您上次拖拽停靠的屏幕坐标，重启后无缝复原。
- 🛡️ **深层容错引擎**：精准拦截系统底层致命错误（HTTP 400 网络中断、模型崩溃等），并瞬间同步给宠物，拒绝长时间卡死在“思考中”。
